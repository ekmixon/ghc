            ...
            s | isTrue# (s ># 52#) -> (0, v)
              | m < 0                 ->
                case integerToInt64# (integerNegate m) of
                  n ->
                    case n `uncheckedIShiftRA#` s of
                      k ->
                        (integerFromInt64# (negateInt64# k),
                          case subInt64# n (k `uncheckedIShiftL#` s) of
                            r ->
        ...
        | isTrue# (e <# 0#) ->
          case negateInt# e of
            s | isTrue# (s ># 52#) -> if m < 0 then (-1) else 0
              | otherwise          ->
                case integerToInt64# m of
                  n -> integerFromInt64# (n `uncheckedIShiftRA#` s)
...
double2Integer (D# x) =
    case integerDecodeDouble# x of
      (# m, e #)
        | isTrue# (e <# 0#) ->
          case integerToInt64# m of
            n -> integerFromInt64# (n `uncheckedIShiftRA#` negateInt# e)
        | otherwise -> integerShiftL# m (int2Word# e)
