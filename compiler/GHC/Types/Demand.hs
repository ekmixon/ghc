...
In some cases, we may even unbox record fields of strictly used arguments that
are not itself used strictly by the function. For example, that is the case
when the field is a strict field, see Note [Unbox evaluated arguments] in
"GHC.Core.Opt.WorkWrap.Utils". As far as the ability to unbox is concerned,
*evaluatedness* of the argument is the important trait, and being strict
in an argument means we may evaluate it before entering the function body
(call-by-value), implying evaluatedness.
...
box, in a process called "reboxing". This makes a large difference if the call
site already had 'h' available boxed, for example `case ... of Just h -> ann h`.

We perform a simple syntactic boxity analysis alongside demand analysis in order
to determine whether the box of a strict argument is always discarded in the
function body, in which case we can pass it unboxed without risking regressions
such as in 'ann' above.
...
The 'ann' example demonstrates what happens if we let 'Unboxed' from the first
clause win over 'Boxed' from the second clause (so @Boxed < Unboxed@).
Moreover, there have been attempts to measure the impact of letting Unboxed win;
see Note [Boxed should win].

This boxity analysis is a bit too conservative for recursive functions, though.
Consider
```
sum z      []          = z
...
as decribed in Note [Unboxed demands on recursive function bodies] in
"GHC.Core.Opt.DmdAnal".

It is quite important that our boxity analysis anticipates the actual unboxing
decision of the worker/wrapper transformation,
see Note [Demand signature and worker/wrapper should agree on Boxity] in
"GHC.Core.Opt.DmdAnal".
...
demand (cf. Note [Demand signature and worker/wrapper should agree on Boxity]),
